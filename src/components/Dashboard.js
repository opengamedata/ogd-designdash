...
export default function Dashboard() {
  ...
  const generateGrid = (row, col) => {
    ...
    for (let r = 1; r <= row; r++) {
      for (let c = 1; c <= col; c++) {
        grid.push(
          <VizContainer column={c} row={r} key={`row-${r}-col-${c}`} />
        );
      }
    }
  ...
  return (
    <div className="container flex flex-col gap-4">
      ...
      <div id="Description" className="h-fit max-w-xl m-4">
        ...
        <p>
          A visualization tool for you to intuitively interpret data collected
          from gameplays. Pick a game and a time range to begin.
        </p>
      </div>
      ...
      <div
        ...
        className="relative grid grid-flow-row-dense"
      >
