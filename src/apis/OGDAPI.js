...
export class OGDAPI {
  ...
  static fetch(request) {
    ...
    if (request != null) {
      ...
      if (!localData) {
        ...
        ret_val = fetch(url, options)
          .catch((error) => {
            console.warn(
              `Could not make API Request, got the following error:\n${error}`
            );
          ...
          .catch((error) => {
            console.error(
              `Could not parse JSON from API response, got the following error:\n${error}`
            );
        ...
        ret_val = new Response(localData, { status: 200 })
          ...
          .catch((error) => {
            console.log(
              `Could not get JSON from cached response object, an error occurred!\n${error}`
            );
    ...
    console.log("OGDAPI.fetch() ret_val", ret_val);
    return ret_val
      .then((response) => new APIResponse(response))
      .catch((error) => {
        console.log(
          `Could not convert Response JSON to an APIResponse object, an error occurred!\n${error}`
        );
